...
const TooltipContent: FC<TooltipContentProps> = ({ tooltipInfo }: TooltipContentProps) => {
  ...
  const layerDownloads = useMemo(() => {
    ...
    downloadTemplates = downloadTemplates?.map((template) => ({
      ...template,
  ...
  return (
    <div
      className="p-3 bg-navyblue z-50 text-white border border-mainblue text-xs"
      style={{ width: '240px' }}
    >
      ...
      <div className="text-mainblue mb-2">
        <div className="text-mainblue mb-1">{layerLabel}</div>
        <div className="text-mainblue">{tooltipInfo && parseToUTC(acqtime)}</div>
      </div>
        ...
        {layerDownloads
          ...
          .map((layerDownload) => (
            <div className="mb-1 text-white underline">
              <a href={layerDownload.image} target="_blank" title={layerDownload.caption}>
                {layerDownload.caption}
              </a>
            </div>
          ))}
      </div>
