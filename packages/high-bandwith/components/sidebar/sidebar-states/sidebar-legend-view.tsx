...
const SidebarLegendView: FC<SideBarLegendViewProps> = () => {
  ...
  const activeLayers = layerGroups
    .map((layerGroup) => {
      const layerGroupActiveLayers = layerGroup.layers
        .map((layer) => {
          if (!layer.checked) return null;
          if (isGroupedLayer(layer))
            return layer.layers.find((groupedLayer) => groupedLayer.checked) || null;
          return layer;
        })
        .filter(Boolean);
  ...
  const renderLayerLegend = (layerGroup) => {
    return Object.keys(layerGroup).map((layerGroupId: string) => (
        ...
        {layerGroup[layerGroupId].map((layer: Layer) => (
          <div key={`legend-item-${layerGroupId}`}>
            <div className="text-mainblue">
              <span
                className={cx('w-2 h-2 mr-2 ml-1 inline-block rounded-full border', {
