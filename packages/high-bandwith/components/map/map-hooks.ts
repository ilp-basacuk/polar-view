...
export const useLayerManager = ({
  ...
  useEffect(() => {
    ...
    const addLayerToMap = (layerConfig) => {
      ...
      const layerSource = new LeafletWmsSource(layerConfig.url, {
    ...
    const removeLayers = (layerIds: string[]) => {
      ...
      const layerIdsToRemove = activeLayerIds.filter(
        (id) => !basemapIds.includes(id) && !layerIds.includes(id),
      );
    ...
    const updateLayerParams = () => {
      ...
      if (Object.keys(paramsToChange).length) {
        Object.keys(paramsToChange).forEach((layerId) => {
          sources[layerId]._overlay.setParams({
            ...sources[layerId].options.params,
            ...paramsToChange[layerId],
          });
        });
      }
    };
