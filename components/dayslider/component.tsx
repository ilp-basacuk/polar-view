...
const DaySlider: React.FC<IDaySlider> = ({ startDate, onChange, ...props }) => {

  const onDayChange = React.useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    if (onChange) {
      onChange(addDays(startDate, parseInt(event.target.value)));
    }
  }, []);
        ...
        {Days.map((val) => {
          return (
            <div className="relative border border-mainblue bg-softerblue flex-1 cursor-pointer">
              ...
              />
              <div className="h-3 w-full cursor-pointer"></div>
            </div>
          );
        })}
      </div>
